	...
	if(dialogid == DIALOG_INTERIORS)
	    {
	        if(response)
	        {
	            SetPlayerPos(playerid, interiorArray[listitem][intX], interiorArray[listitem][intY], interiorArray[listitem][intZ]);
				SetPlayerFacingAngle(playerid,interiorArray[listitem][intA]);
				SetPlayerInterior(playerid,interiorArray[listitem][intID]);
				PlayerInfo[playerid][pInt] = interiorArray[listitem][intID];
...
CMD:interiors(playerid, params[])
{
	...
	ShowPlayerDialogEx(playerid, DIALOG_INTERIORS, DIALOG_STYLE_LIST, "Choose an interior to teleport to.", list, "Select", "Cancel");
	return 1;
}