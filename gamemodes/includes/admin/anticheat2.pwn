...
AC_Flag(playerid, processid, iExtraID = INVALID_PLAYER_ID, Float:fInfo = 0.0) {
	...
	switch(processid) {
		...
		case AC_PROAIM: {
			...
			if(iInfo > 6) AC_Process(playerid, AC_PROAIM, _:iExtraID);
			else { 
		...
		case AC_AIMBOT: {
			if(fInfo > 20) AC_Process(playerid, AC_AIMBOT, _:iExtraID);
			else {
	...
	//if(arrAntiCheat[playerid][ac_iFlags][processid] == 1 || arrAntiCheat[playerid][ac_iFlags][processid] % 5 == 0) { // prevent spamming
	{
		...
		switch(processid) {
			...
			case AC_SILENTAIM: {
				...
				format(szMiscArray, sizeof(szMiscArray), "[SYSTEM]: %s (%d) used Silent Aim (detector %d) on %s (%d) with a %s (warnings: %d).", GetPlayerNameExt(playerid), PlayerInfo[playerid][pId], iExtraID3, GetPlayerNameExt(iExtraID2), PlayerInfo[iExtraID2][pId], AC_GetWeaponName(_:iExtraID), arrAntiCheat[playerid][ac_iFlags][processid]);
				Log("logs/anticheat.log", szMiscArray);
				
				/*
				if(arrWeaponDataAC[plmysql_tquery(MainPipeline, szQuery, "OnQueryFinish", "i", SENDDATA_THREAD);AC[playerid][ac_iBulletsFired][iExtraID] - arrWeaponDataAC[playerid][ac_iBulletsHit][iExtraID],
						iRelevantMiss = arrWeaponDataAC[playerid][ac_iBulletsFired][iExtraID] - arrWeaponDataAC[playerid][ac_iBulletsHit][iExtraID] - arrWeaponDataAC[playerid][ac_iFakeMiss][iExtraID];
				...
				*/
				...
				format(szMiscArray, sizeof(szMiscArray), "{AA3333}[SYSTEM]: {FFFF00}%s is using Silent Aim (detector %d) on %s with a %s (warnings: %d).", GetPlayerNameExt(playerid), iExtraID3, GetPlayerNameExt(iExtraID2), AC_GetWeaponName(_:iExtraID), arrAntiCheat[playerid][ac_iFlags][processid]);
			}
			case AC_PROAIM: {
			...
			case AC_HEALTHARMORHACKS: {
				...
				SetTimerEx("KickEx", 1000, 0, "i", _:iExtraID);
